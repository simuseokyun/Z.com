...
export default function UserInfo({ username, session }: Props) {
    ...
    const follow = useMutation({
        mutationFn: async (userId: string) => {
            return await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/users/${userId}/follow`, {
                credentials: 'include',
                cache: 'no-store',
                method: 'post',
            });
        ...
        onError(error, userId) {
            ...
            if (value) {
                ...
                if (index > -1) {
                    shallow[index] = {
                        ...shallow[index],
                        Followers: shallow[index].Followers.filter((follower) => follower.id !== session?.user?.email),
                        _count: { ...shallow[index]._count, Followers: shallow[index]._count?.Followers - 1 },
                    };
                }
                queryClient.setQueryData(['users', 'followRecommends'], shallow);
            }
            ...
            if (value2) {
                const shallow = {
                    ...value2,
                    Followers: [{ id: session?.user?.email as string }],
                    _count: { ...value2._count, Followers: value2._count.Followers + 1 },
                    ...
                    <div className={style.userName}>
                        ...
                        {session?.user?.email !== username && (
                            <>
                                ...
                                <button
                                    className={cx(style.followButton, followed && style.following)}
                                    onClick={onFollow}
                                >
                                    {followed ? '팔로잉' : '팔로우'}
                                </button>
