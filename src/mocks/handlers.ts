...
export const handlers = [
    ...
    http.post(`${baseUrl}/api/login`, () => {
        return HttpResponse.json(User[0], { headers: { 'Set-Cookies': 'connect.sid=msw-cookies;HttpOnly;Path=/' } });
    }),
    http.post(`${baseUrl}/api/logout`, () => {
        return HttpResponse.text('로그아웃', {
            headers: {
    ...
    http.post(`${baseUrl}/api/users`, () => {
        return HttpResponse.text(JSON.stringify('성공입니다'));
    }),
    ...
    // http.get(`${baseUrl}/api/followingPots`, ({ request }) => {
    //     return HttpResponse.json([
    //         {
    //             postId: 1,
    //             User: User[0],
